const defaultCustomColors = {
  bg1: '#141414', bg2: '#1d1d1d', bg3: '#262626',
  txt1: '#dedede', txt2: '#aeaeae', txt3: '#636363',
  txt4: '#484848', txtErr: '#d6809c',
}
...
function loadSettings() {
  try {
    ...
    if (stored) {
      ...
      return { ...defaultSettings, ...parsed, customThemeColors: { ...defaultCustomColors, ...(parsed.customThemeColors || {}) } }
    }
  } catch {}
...
export function saveSettings(s) {
  try { localStorage.setItem('startpage-settings', JSON.stringify(s)) } catch {}
}
...
export function exportSettings() {
  ...
  const a = document.createElement('a'); a.href = url; a.download = 'startpage-settings.json'; a.click()
...
export function importSettings(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      const merged = { ...defaultSettings, ...parsed, customThemeColors: { ...defaultCustomColors, ...(parsed.customThemeColors || {}) } }
